...
### 概要
- これまではGitHub ActionsからGoogle CloudのArtifact Registryなどに認証する際にService Accountの発行が必要であった
- しかし、これは管理が複雑になるだけでなく、キーの漏洩リスクも孕んでいる
- そこで利用するのが、Workload Identity 連携を使用した GitHub Actions から Google Cloud への認証
...
1. GitHub OIDCトークン発行サーバー（OIDCプロバイダー）からGitHub OIDCトークンを発行する
