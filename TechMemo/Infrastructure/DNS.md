...
- `キャッシュ`と`ネガティブキャッシュ`
    - 「存在しない」という結果もキャッシュする（`ネガティブキャッシュ`）
        ...
        - e.g. 存在しないドメインにアクセスした場合、何度もリロードするかも。その場合に、毎回問い合わせるのは無駄すぎる
- 複数代の権威サーバーを設置する
    - 同じゾーンデータを持つ権威サーバーを複数台設置すると、どの権威サーバーに問い合わせても同じ応答を得ることができる
    - DNSでは、この仕組みを`ゾーン転送`で実現できる
        - ゾーン転送では、ゾーンデータを持つ権威サーバーがコピー元となり、コピー先の権威サーバーはコピー元からゾーンデータを受け取る
        - コピー元となる権威サーバーを`プライマリサーバー`、コピー先となる権威サーバーを`セカンダリサーバー`と言う
- フルリゾルバーにおける権威サーバーの選択
    - 応答時間（`RTT=Round Trip Time`）が短いものを選択
...
- `自分のドメイン名を、インターネットで使えるようにする`
    - 自分のゾーンを管理する権威サーバーを動かす必要がある
        1. 自分のドメイン名をどう管理するかを決める
        2. 自分のゾーンを管理する権威サーバーを設定・公開・運用する
- `インターネットで使われているドメイン名を、自分が使えるようにする`
    - 自組織の利用者が使うフルリゾルバーを動かし、名前解決サービスを提供する必要がある
- `DNSを動かし続け、可用性を高める`

## 🌟EXAMPLE社を例にした設計・構築

1. 会社のドメイン名としてexample.co.jpを登録し、システム部門で管理する

- example.co.jpのゾーン情報を公開するための権威サーバーを用意し、インターネットからアクセスできるようにする必要がある
- jpゾーンの管理者がEXAMPLE社にexample.co.jpというサブドメインを委任する形になるため、具体的には以下のように進む
    1. jpが委任するサブドメインはexample.co.jpである
    2. 委任元（jp）は、EXAMPLE社のシステム部門の管理者がexample.co.jpの管理者であることを確認する
    3. 委任元は、委任先の管理者であるあなたから（`レジストラ/指定事業者` 経由で）example.co.jpゾーンの権威サーバーの名前とIPアドレスの情報を受け取る
    4. 委任元はjpゾーンにexample.co.jpの委任情報を設定する
- 委任情報は`NSリソースレコード`によって設定する
    - `DNSでは、親ゾーンと子ゾーンの双方に、同じ内容のNSリソースレコードを設定する必要がある`

2. www.example.co.jpという名前でWebサイトを公開し、「ユーザー名@example.co.jp」というメールアドレスを使えるようにする

- EXAMPLE社の公式Webサイトとして、www.example.co.jpというWebサーバーを公開する場合を考える
- そのためには、Webサーバーを用意した後、example.co.jpの権威サーバーにwww.example.co.jpというドメイン名を用意し、IPアドレスの情報を設定する必要がある
- IPはIPv4とIPv6の2種類があるため、必要に応じて`Aリソースレコード(IPv4アドレス)`と`AAAAリソースレコード(IPv6アドレス)`を設定する
- これにより、権威サーバーはフルリゾルバーにwww.example.co.jpのIPアドレスを教えることができるようになり、EXAMPLE社の公式Webサイトを公開できる

3. 社内の各部門のために、サブドメインを用意する
...
- ゾーン転送の仕組み
    - ゾーン転送には以下の2種類がある
        ...
        - `IXFR(Incremental Transfer)`
            - あるバージョンからの差分のみを送る
            - `時間と負荷を下げるため、こちらを使うのが基本`
    - プライマリサーバーは`DNS NOTIFY`と呼ばれる仕組みを用いて、ゾーンデータの更新があったことをセカンダリサーバーに伝える

## サービスを提供するために設定する情報

- IPにはIPv4とIPv6の2種類があるため、必要に応じて`Aリソースレコード`と`AAAAリソースレコード`を設定する
